...
AddrSpace::AddrSpace(OpenFile *executable, PCB* newPCB)
{
    ...
    //new addition
    this->disk_location = new int[numPages];
	...
	//new addition
	disk_location[i] = virtualMemoryManager->allocSwapSector();
        ...
        virtualMemoryManager->writeToSwap(placeHolder, PageSize, disk_location[i]);

        // Debuggin output
        int currVirtPage = disk_location[i] / PageSize;
...
AddrSpace::AddrSpace(const AddrSpace* other, PCB* newPCB)
{
    ...
    for (unsigned int i = 0; i < numPages; i++) { 
        ...
        // Allocate space for entire addr space on backing store at creation
        //pageTable[i].space = this;
        this->disk_location[i] = virtualMemoryManager->allocSwapSector();
        virtualMemoryManager->copySwapSector(disk_location[i], other->disk_location[i]);
                // Maintain swap space page information
        //SwapSectorInfo * swapInfo =
        //       virtualMemoryManager->getSwapSectorInfo(pageTable[i].locationOnDisk / PageSize);
        //swapInfo->pageTableEntry = &pageTable[i];
...
int AddrSpace::ReadFile(int virtAddr, OpenFile* file, int size, int fileAddr)
{
    ...
    while (size > 0) { // we have remaining bytes to read
        ...
        virtualMemoryManager->writeToSwap(buffer1+bytesCopiedSoFar, numBytesThisLoop, disk_location[pageTableIndex]+offset);
