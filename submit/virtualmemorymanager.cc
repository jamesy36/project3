...
void VirtualMemoryManager::swapPageIn(int virtAddr)
{
    ...
    */
        ...
        TranslationEntry* pageEject = physPageInfo->space->getPageTableEntry(physPageInfo->pageTableIndex);
       while(pageEject->use){
        pageEject->use = false;
        nextVictim = (nextVictim + 1) % NumPhysPages;
        physPageInfo = physicalMemoryInfo + nextVictim;
        pageEject = physPageInfo->space->getPageTableEntry(physPageInfo->pageTableIndex);
        }
        pageEject->valid = false;
        if(pageEject->dirty){
            writeToSwap(machine->mainMemory + (pageEject->physicalPage * PageSize), PageSize, physPageInfo->space->disk_location[physPageInfo->pageTableIndex]);
        }
        ...
        currPageEntry->physicalPage = pageEject->physicalPage;
    }
    else{
        physPageInfo->space = currentThread->space;
        physPageInfo->pageTableIndex = virtAddr/PageSize;
...
//      swapPageInfo->removePage(currPage);
      //swapPageInfo->pageTableEntry = NULL;
        ...
        swapSectorMap->Clear((space->disk_location[i]) / PageSize);
    }
}
...
void VirtualMemoryManager::loadPageToCurrVictim(int virtAddr)
{
    ...
    int swapSpaceLoc = currentThread->space->disk_location[pageTableIndex];
